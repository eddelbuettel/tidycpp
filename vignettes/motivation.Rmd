...
SEXP out(SEXP x, SEXP y)
{
  ...
  SEXP ans =
    R::Protect(R::allocMatrixReal(nx, ny));
...
- no `PROTECT` and `UNPROTECT` with manual calling of the number of calls made: C++ takes care of
  that for us via `Protect` which is a modernized (and simplified) version of class `Shield` in Rcpp;
...
SEXP attribute_hidden do_detect_vec(SEXP x, T fun) {
  ...
  SEXP res = R::Protect(R::allocVectorCharacter(n));
...
SEXP detect_raw(SEXP x) {
  ...
  SEXP res = R::Protect(R::allocVectorCharacter(1));
  uchardet_t handle = uchardet_new();
